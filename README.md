...
## 示例13: 最后一个string字段允许包含分隔符
默认情况下,message的多个字段值之间使用分隔符(如`_`)进行分隔,但当某个string字段的内容本身也包含该分隔符时,会导致解析错误。
针对这种情况,如果message的**最后一个字段是string类型**,导表工具会自动将多余的分隔内容合并到该字段中,无需使用`#Sep`切换分隔符。
...
---------------------------------------------------------------------------
| Id  | Data               | Datas                            |
|     | #Field=no          | #Field=Key_Value                 |
---------------------------------------------------------------------------
| 1   | key1_a_b_c         | k1_v1_x;k2_v2_y_z                |
---------------------------------------------------------------------------
| 2   | hello_world_foo    | a_1;a_b_c_d_2                    |
---------------------------------------------------------------------------
```
...
{
  "Id": 2,
  "Data": {"Key": "hello", "Value": "world_foo"},
  "Datas": [
    {"Key": "a", "Value": "1"},
    {"Key": "a_b_c_d", "Value": "2"}
  ]
...
说明:
- 当分隔后的字段数量超过message定义的字段数量时,会检查最后一个字段是否为string类型
- 如果最后一个字段是string类型,则使用限定次数的分割,把剩余的所有内容(含分隔符)整体作为该字段的值
- 该特性适用于`#Field=no`(按字段顺序)和`#Field=Field1_Field2`(指定字段名)两种格式
- 只有**最后一个字段**是string时才生效,中间字段仍不允许包含分隔符
- 适用于配置键值对、路径、富文本等本身包含分隔符的内容,避免为了规避冲突而频繁切换`#Sep`
